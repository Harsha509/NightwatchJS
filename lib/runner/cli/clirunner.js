...
var SETTINGS_DEPRECTED_VAL = './saucelabs.json';
...
CliRunner.prototype = {
  ...
  readSettings : function() {
    // use default nighwatch.json file if we haven't received another value
    ...
    if (this.cli.command('config').isDefault(this.argv.config)) {
      ...
      if (fs.existsSync(SETTINGS_JS_FILE)) {
        ...
        var binFolder = path.resolve(__dirname + '/../../../configurations');
    ...
    switch (testRunnerType) {
      case 'default':
        Runner.run(source, this.test_settings, {
          output_folder : './reports/recent_reports/xmlfiles/',
