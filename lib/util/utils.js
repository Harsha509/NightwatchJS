...
Util.getScreenshotFileName = function(currentTest, is_error, screenshots_path) {
 ...
 // prefix += is_error ? '_ERROR' : '_FAILED';
  ...
  return path.resolve(path.join(screenshots_path, prefix + '.png'));
...
Util.showStackTrace = function(headline, stack, isErr) {
  ...
  if (stack) {
    console[logMethod](Logger.colors.light_gray(stackTrace));
  }
};